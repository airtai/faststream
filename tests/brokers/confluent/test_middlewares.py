...
class LocalMiddlewareTestcase:
    ...
    async def test_local_middleware(
        ...
        @broker.subscriber(queue, middlewares=(mid,), auto_offset_reset="earliest")
        async def handler(m):
            event.set()
            return ""
    ...
    async def test_local_middleware_not_shared_between_subscribers(
        ...
        @broker.subscriber(
            queue + "1", middlewares=(mid,), auto_offset_reset="earliest"
        )
        async def handler(m):
            if event1.is_set():
                event2.set()
    ...
    async def test_patch_publish(self, queue: str, mock: Mock, event, raw_broker):
        ...
        @broker.subscriber(queue, middlewares=(Mid, Mid), auto_offset_reset="earliest")
        async def handler(m):
            return "r"

        @broker.subscriber(queue + "r", auto_offset_reset="earliest")
