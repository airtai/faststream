...
# %% auto 0
__all__ = ['logger', 'ProduceCallable', 'TopicCallable', 'KafkaErrorMsg', 'FastKafkaAPI', 'produce_decorator',
           'populate_consumers', 'populate_producers']
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 6
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 9
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 12
@patch
def _add_topic(
    self: FastKafkaAPI,
    *,
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 14
def _get_first_func_arg_type(f: Callable[[Any], Any]) -> Type[Any]:
    return list(get_type_hints(f).values())[0]

# %% ../nbs/FastKafkaAPI_rework.ipynb 16
def produce_decorator(self, topic, func):
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 19
@patch
# todo: razbi na dvije metode, tj. vrati ih nazad u produces i consumes metode
def _register_kafka_callback(
    self: FastKafkaAPI,
    *,
    store_key: str,
    ...
    **kwargs,
    ...
    def _decorator(
        ...
        if produces and type(kwargs.get("producer")) == AIOKafkaProducer:
            on_topic = AIOKafkaProducer

        self._add_topic(
            store_key=store_key, topic=topic, f=on_topic, on_error=on_error, **kwargs
        )

        return produce_decorator(self, topic, on_topic) if produces else on_topic
    ...
    tx = [
        asyncio.create_task(
            aiokafka_consumer_loop(
                topics=[topic],
                bootstrap_servers=configuration.get(
                    "bootstrap_servers", config["bootstrap_servers"]
                ),
                auto_offset_reset=configuration.get(
                    "auto_offset_reset", config["auto_offset_reset"]
                ),
                max_poll_records=configuration.get(
                    "max_poll_records", config["max_poll_records"]
                ),
                max_buffer_size=configuration.get(
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 26
# TODO: Add passing of vars


def populate_producers(*, app: FastKafkaAPI) -> None:
    config: Dict[str, Any] = app._kafka_config
    print(app._store["producers"])
    for topic, (potential_producer, configuration) in app._store["producers"].items():
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 27
@patch
def _on_startup(self: FastKafkaAPI) -> None:
    ...
    self._confluent_producer = AIOKafkaProducer(
        bootstrap_servers=app._kafka_config["bootstrap_servers"]
    )
    asyncio.create_task(self._confluent_producer.start())
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 48
@patch
...
# %% ../nbs/FastKafkaAPI_rework.ipynb 50
@patch
@asynccontextmanager
