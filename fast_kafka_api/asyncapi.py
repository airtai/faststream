...
# %% auto 0
__all__ = ['logger', 'sec_scheme_name_mapping', 'T', 'KafkaMessage', 'SecurityType', 'APIKeyLocation', 'SecuritySchema',
           'KafkaBroker', 'ContactInfo', 'KafkaServiceInfo', 'KafkaBrokers', 'export_async_spec']
...
# %% ../nbs/003_AsyncAPI.ipynb 13
class KafkaBrokers(BaseModel):
    brokers: Dict[str, KafkaBroker]

# %% ../nbs/003_AsyncAPI.ipynb 16
T = TypeVar("T")


def _get_msg_cls_for_method(f: Union[Callable[[T], Any], Callable[[T, Any], Any]]) -> T:
    classes = list(get_type_hints(f).values())

    # @app.produces callback func accepts msg and kafka_msg
    if len(classes) > 2:
        raise ValueError(classes)
    return classes[0]

# %% ../nbs/003_AsyncAPI.ipynb 18
def _get_topic_dict(
    ...
    msg_cls = _get_msg_cls_for_method(f)
...
# %% ../nbs/003_AsyncAPI.ipynb 22
def _get_kafka_msg_classes(
    consumers: Dict[str, Callable[[KafkaMessage], None]],
    producers: Dict[str, Callable[[KafkaMessage, Any], None]],
) -> Set[KafkaMessage]:
    fx = list(consumers.values()) + list(producers.values())
    return set([_get_msg_cls_for_method(f) for f in fx])
