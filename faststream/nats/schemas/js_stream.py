...
class JStream(NameRequired):
    ...
    def __init__(
        ...
        max_msgs_per_subject: Annotated[
            int,
            Doc("Limit message count per every unique subject. Stream index subjects to it's pretty fast tho.-")
        ] = -1,
        max_msg_size: Annotated[
            Optional[int],
            Doc("Limit message size to be received. Note: the whole message can't be larger than NATS Core message "
                "limit.")
        ] = -1,
        storage: Annotated[
        ...
        placement: Annotated[
            Optional["Placement"],
            Doc("NATS Cluster for stream to be deployed in. Value is name of that cluster.")
        ] = None,
        mirror: Annotated[
            Optional["StreamSource"],
            Doc("Should stream be read-only replica of another stream, if so, value is name of that stream.")
        ] = None,
        sources: Annotated[
            Optional[List["StreamSource"]],
            Doc("Should stream mux multiple streams into single one, if so, values is names of those streams")
        ] = None,
        sealed: Annotated[
            bool,
