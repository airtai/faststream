...
from faststream.specification.asyncapi.utils import clear_json_key
...
def get_broker_channels(
    ...
    for h in broker._subscribers.values():
        schema = h.schema()
        channels.update(
            {clear_json_key(key): channel_from_spec(channel) for key, channel in schema.items()}
        )
...
def _resolve_msg_payloads(
    ...
    if isinstance(one_of, dict):
        for p_title, p in one_of.items():
            p_title = clear_json_key(p_title)
    ...
    message_title = clear_json_key(m.title)
