...
class APIKeyLocation(str, Enum):
    """An enumeration of API key locations.

    Attributes:
        user : API key is in the user field.
        password : API key is in the password field.
        query : API key is in the query string.
        header : API key is in the request header.
        cookie : API key is in the request cookie.
...
class SecuritySchema(BaseModel):
    """A class to represent a security schema.

    Attributes:
        security_type : type of the security scheme
        description : description of the security scheme
        name : name of the security scheme
        api_key_loc : location of the API key
        scheme : scheme
...
class ContactInfo(BaseModel):
    ...
    Attributes:
        ...
        url (HttpUrl): url of the contact
...
class KafkaServiceInfo(BaseModel):
    """A class to represent a Kafka service info.

    Attributes:
        title (str): Title of the service
        version (str): Version of the service
        description (str): Description of the service
        contact (ContactInfo): Contact info of the service
...
class KafkaBrokers(BaseModel):
    """A class to represent a set of Kafka brokers.

    Attributes:
        brokers : a dictionary of Kafka brokers
...
def _get_msg_cls_for_producer(f: ProduceCallable) -> Type[Any]:
    """Get the message class for a producer

    Args:
        f: A producer function

    Returns:
        The message class

    Raises:
        ValueError: If the producer function does not have a return value or does not have a json method
...
def _get_msg_cls_for_consumer(f: ConsumeCallable) -> Type[Any]:
    """Get the message class for a consumer function.

    Args:
        f: The consumer function.

    Returns:
        The message class.

    Raises:
        ValueError: If the consumer function has more than one input param or returns a value.
...
def _get_topic_dict(
    ...
    """Get the topic dict for a function.

    Args:
        f: The function
        direction: The direction of the topic.

    Returns:
        The topic dict

    Raises:
        ValueError: If direction is not one of ['publish', 'subscribe']
...
def _get_channels_schema(
    ...
    """Get the channels schema.

    Args:
        consumers: A dictionary of consumers.
        producers: A dictionary of producers.
...
def _get_kafka_msg_definitions(
    ...
    """Get the Kafka message definitions for the given consumers and producers.

    Args:
        consumers: A dictionary of consumers, where the key is the consumer name and the value is the consumer function.
        producers: A dictionary of producers, where the key is the producer name and the value is the producer function.

    Returns:
        A dictionary of Kafka message definitions, where the key is the message name and the value is the message definition.
...
def _get_msg_definitions_with_examples(
    ...
    """Get message definitions with examples

    Args:
        consumers: A dictionary of consumer functions
        producers: A dictionary of producer functions

    Returns:
        A dictionary of message definitions with examples
...
def _get_security_schemes(kafka_brokers: KafkaBrokers) -> Dict[str, Any]:
    """Get the security schemes for the Kafka brokers.

    Args:
        kafka_brokers: The Kafka brokers

    Returns:
        The security schemes for the Kafka brokers
...
def _get_components_schema(
    ...
    def _sub_values(d: Any, substitutions: Dict[str, str] = substitutions) -> Any:
        """Substitute values in a dictionary or list of dictionaries.

        Args:
            d: A dictionary or list of dictionaries
            substitutions: A dictionary of substitutions

        Returns:
            The substituted dictionary or list of dictionaries
...
def _get_asyncapi_schema(
    ...
    Args:
        ...
        kafka_brokers: The Kafka brokers.
        kafka_service_info: The Kafka service info.
...
def _generate_async_spec(
    *,
    ...
    # generate spec file
    """Generate async spec file

    Args:
        consumers: A dictionary of consumers
        producers: A dictionary of producers
        kafka_brokers: Kafka brokers
        kafka_service_info: Kafka service info
        spec_path: Path to the spec file
        force_rebuild: Force rebuild

    Returns:
        True if the spec file is changed, False otherwise
...
def _install_deps() -> None:
    """Install AsyncAPI generator dependencies
    ...
    Raises:
        ValueError: If AsyncAPI generator is not installed
...
def _generate_async_docs(
    *,
    ...
    """Generate async docs from a spec file.

    Args:
        spec_path: Path to the spec file
        docs_path: Path to the docs file

    Raises:
        ValueError: If the generation of async docs fails
