...
# %% auto 0
__all__ = ['logger', 'sec_scheme_name_mapping', 'timedelta_to_isoformat', 'KafkaMessage', 'SecurityType', 'APIKeyLocation',
...
def timedelta_to_isoformat(duration: timedelta) -> str:
    ...
    Example:
        >>> duration = timedelta(days=1, hours=2, minutes=30, seconds=15, microseconds=500)
        >>> timedelta_to_isoformat(duration)
        'P1DT2H30M15.000500S'
    """
    ...
    iso_duration = f"P{days}DT{hours}H{minutes}M{seconds}"
    if microseconds:
    ...
    if is_negative:
        iso_duration = f"-{iso_duration}"

    return iso_duration

# %% ../../nbs/014_AsyncAPI.ipynb 7
class KafkaMessage(BaseModel):
    # ToDo: ser_json_timdelta to iso8601 format is not working
    model_config = ConfigDict(ser_json_timedelta="iso8601")

    @model_serializer
    def serialize_model(self) -> Dict[str, Any]:
        """This method is used to properly format timedelta in ISO format because the default ConfigDict(ser_json_timedelta="iso8601") is not working"""
        return {
            x: timedelta_to_isoformat(getattr(self, x))
            if isinstance(getattr(self, x), timedelta)
            else getattr(self, x)
