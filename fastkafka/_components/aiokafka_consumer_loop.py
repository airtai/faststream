...
# %% auto 0
__all__ = ['logger', 'P', 'sanitize_kafka_config', 'aiokafka_consumer_loop']
...
) -> Callable[[BaseModel, Any], Awaitable[None]]:
    async def _params_wrap(  # type: ignore
        msg: BaseModel,
        meta: ConsumerRecord,
        callback: Callable[P, Awaitable[None]] = callback,
    ) -> None:
        types = list(get_type_hints(callback).values())
        args: P.args = [msg]
        if ConsumerRecord in types:
            args.insert(types.index(ConsumerRecord), meta)
        await callback(*args)

    return _params_wrap

# %% ../../nbs/011_ConsumerLoop.ipynb 14
def _create_safe_callback(  # type: ignore
    callback: Callable[P, Awaitable[None]]
) -> Callable[[BaseModel, ConsumerRecord], Awaitable[None]]:
    """
    ...
    async def _safe_callback(  # type: ignore
        msg: BaseModel,
        meta: ConsumerRecord,
        original_callback: Callable[P, Awaitable[None]] = callback,
        callback: Callable[
            [BaseModel, ConsumerRecord], Awaitable[None]
        ] = _callback_parameters_wrapper(callback),
    ) -> None:
        try:
            await callback(msg, meta)
        except Exception as e:
            logger.warning(
                f"_safe_callback(): exception caugth {e.__repr__()} while awaiting '{original_callback}({msg})'"
            )
...
def _prepare_callback(
    callback: Callable[P, Union[None, Awaitable[None]]]
) -> Callable[[BaseModel, Any], Awaitable[None]]:
    """
...
async def _aiokafka_consumer_loop(  # type: ignore
    ...
    **kwargs: Any,
    ...
    async def process_message_callback(
        ...
        callback: Callable[[BaseModel, Any], Awaitable[None]] = prepared_callback,
            ...
            try:
                async for record in _streamed_records(receive_stream):
                    try:
                        ...
                        await callback(decoded_msg, record)
                    except Exception as e:
                        logger.warning(
