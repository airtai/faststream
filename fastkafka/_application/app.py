...
# %% auto 0
__all__ = ['logger', 'OverrideBrokers', 'FastKafka', 'AwaitedMock']
...
OverrideBrokers = namedtuple("OverrideBrokers", ["name", "brokers"])
...
def consumes(
    ...
    *,
    ...
    kafka_brokers: Optional[OverrideBrokers] = None,
    prefix: str = "on_",
    **kwargs: Dict[str, Any],
    ...
    def _decorator(
        ...
        kafka_brokers: Optional[OverrideBrokers] = kafka_brokers,
        ...
        if kafka_brokers is not None:
            prepared_brokers = OverrideBrokers(
                name=kafka_brokers.name,
                brokers=_get_kafka_brokers(kafka_brokers.brokers),
            )
            if (
                prepared_brokers.brokers.brokers.keys()
                != self._kafka_brokers.brokers.keys()
            ):
                raise ValueError(
                    f"To override application default brokers, you must define all of the broker options. Default defined: {set(self._kafka_brokers.brokers.keys())}, override defined: {set(prepared_brokers.brokers.brokers.keys())}"
                )
            key_prefix = f"_{kafka_brokers.name}"
        else:
            prepared_brokers = None
            key_prefix = ""

        decoder_fn = _get_decoder_fn(decoder) if isinstance(decoder, str) else decoder

        topic_key = f"{topic_resolved}{key_prefix}"

        self._consumers_store[topic_key] = (
            on_topic,
            decoder_fn,
            executor,
            prepared_brokers,
...
def _populate_consumers(
    ...
    self._kafka_consumer_tasks = [
        asyncio.create_task(
            aiokafka_consumer_loop(
                topic=topic.replace(f"_{kafka_brokers.name}", "")
                if kafka_brokers is not None
                else topic,
                ...
                **{
                    ...
                    **{
                        "bootstrap_servers": _get_broker_addr_list(
                            kafka_brokers.brokers.brokers[bootstrap_server]
                            if kafka_brokers is not None
