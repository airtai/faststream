...
def true_after(seconds: float) -> Callable[[], bool]:
    ...
    def _true_after(seconds: float = seconds, t: datetime = t) -> bool:
        """Check if a given time is after a given number of seconds.
        ...
        Returns:
            True if the given time is after the given number of seconds, False otherwise
...
def change_dir(d: str) -> Generator[None, None, None]:
    """Change the working directory to the given directory and return to the previous directory.
...
async def _start(self: LocalKafkaBroker) -> str:
    ...
    async def write_config_and_run(
        ...
        """Write a config file and run a command.

        Args:
            config: The config to write
            config_path: The path to write the config to
