...
Now you can hide your connection secrets in the **AsyncAPI** schema by setting up server URL manually:
...
Also, **RabbitMQ AsyncAPI** schema was improved: added `faststream.security` support, connection scheme defines automatically.

**RabbitMQ** connection parameters are merging now: so, you can define main connection data as an URL string and customize it using kwargs
...
* much suitable `faststream.security` import instead `faststream.broker.security`
